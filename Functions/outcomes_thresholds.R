...
outcomes_thresholds_t2 <- function(cohort_dataset) {
  ...
  p_val_bolus_now_0.8 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_2_misclass_0.8") == 0 | sum(group == "ref_type_2") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_2_misclass_0.8"] == 1), sum(current_bolus_insulin[group == "ref_type_2"] == 1)),
        c(sum(group == "type_2_misclass_0.8"), sum(group == "ref_type_2"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.7 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_2_misclass_0.7") == 0 | sum(group == "ref_type_2") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_2_misclass_0.7"] == 1), sum(current_bolus_insulin[group == "ref_type_2"] == 1)),
        c(sum(group == "type_2_misclass_0.7"), sum(group == "ref_type_2"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.6 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_2_misclass_0.6") == 0 | sum(group == "ref_type_2") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_2_misclass_0.6"] == 1), sum(current_bolus_insulin[group == "ref_type_2"] == 1)),
        c(sum(group == "type_2_misclass_0.6"), sum(group == "ref_type_2"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.5 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_2_misclass_0.5") == 0 | sum(group == "ref_type_2") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_2_misclass_0.5"] == 1), sum(current_bolus_insulin[group == "ref_type_2"] == 1)),
        c(sum(group == "type_2_misclass_0.5"), sum(group == "ref_type_2"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.4 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_2_misclass_0.4") == 0 | sum(group == "ref_type_2") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_2_misclass_0.4"] == 1), sum(current_bolus_insulin[group == "ref_type_2"] == 1)),
        c(sum(group == "type_2_misclass_0.4"), sum(group == "ref_type_2"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  ...
  print(paste0("p-value bolus insulin now for 80/70/60/50/40% thresholds: ", p_val_bolus_now_0.8, ", ", p_val_bolus_now_0.7, ", ", p_val_bolus_now_0.6, ", ", p_val_bolus_now_0.5, ", ", p_val_bolus_now_0.4))
  ...
  dka_post_diag <- cohort_dataset %>%
    ...
    mutate(group=factor(group, levels=c("type_2_misclass_0.8", "type_2_misclass_0.7", "type_2_misclass_0.6", "type_2_misclass_0.5", "type_2_misclass_0.4", "ref_type_2")))
  ...
  treatment_t2_chart_data <- rbind(ins_1_year, bolus_insulin_now) %>% rowwise() %>%
    mutate(label_y = ifelse(!is.na(upper) & (group=="type_2_misclass_0.8"), upper + 7,
                            ifelse(!is.na(upper) & (group=="type_2_misclass_0.7"), upper + 6.5,
                            ifelse(!is.na(upper) & (group=="type_2_misclass_0.6"), upper + 5.8,
                                   ifelse(!is.na(upper) & (group=="type_2_misclass_0.5"), upper + 4,
                                          ifelse(!is.na(upper) & (group=="type_2_misclass_0.4"), upper + 2,
                                                 ifelse(!is.na(upper), upper + 4, NA_real_)))))))
  ...
  plot_file_name <- paste0("C:/Users/ky279/OneDrive - University of Exeter/CPRD/2026/DePICtion/Plots/t2_outcomes_by_threshold.tiff")
...
outcomes_thresholds_t1 <- function(cohort_dataset) {
  ...
  p_val_bolus_now_0.025 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_1_misclass_0.025") == 0 | sum(group == "ref_type_1") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_1_misclass_0.025"] == 1), sum(current_bolus_insulin[group == "ref_type_1"] == 1)),
        c(sum(group == "type_1_misclass_0.025"), sum(group == "ref_type_1"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.05 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_1_misclass_0.05") == 0 | sum(group == "ref_type_1") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_1_misclass_0.05"] == 1), sum(current_bolus_insulin[group == "ref_type_1"] == 1)),
        c(sum(group == "type_1_misclass_0.05"), sum(group == "ref_type_1"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  
  p_val_bolus_now_0.1 <- cohort_dataset %>%
    summarise(p = ifelse(
      sum(group == "type_1_misclass_0.1") == 0 | sum(group == "ref_type_1") == 0,
      NA,
      prop.test(
        c(sum(current_bolus_insulin[group == "type_1_misclass_0.1"] == 1), sum(current_bolus_insulin[group == "ref_type_1"] == 1)),
        c(sum(group == "type_1_misclass_0.1"), sum(group == "ref_type_1"))
      )$p.value)) %>%
    mutate(p=ifelse(p<0.001, "<0.001", round_pad(p, 3))) %>%
    pull(p)
  ...
  print(paste0("p-value bolus insulin now for 2.5/5/10% thresholds: ", p_val_bolus_now_0.025, ", ", p_val_bolus_now_0.05, ", ", p_val_bolus_now_0.1))
  ...
  dka_post_diag <- cohort_dataset %>%
    ...
    mutate(group=factor(group, levels=c("type_1_misclass_0.025", "type_1_misclass_0.05", "type_1_misclass_0.1", "ref_type_1")))
  ...
  treatment_t2_chart_data <- rbind(ins_1_year, bolus_insulin_now) %>% rowwise() %>% mutate(label_y = ifelse(!is.na(upper), upper + 5, NA_real_)) %>% ungroup()
  treatment_t2_chart_data$outcome <- factor(treatment_t2_chart_data$outcome, levels=c("Insulin within 1\nyear of diagnosis", "Current basal-bolus\ninsulin regime"))
  ...
  plot_file_name <- paste0("C:/Users/ky279/OneDrive - University of Exeter/CPRD/2026/DePICtion/Plots/t1_outcomes_by_threshold.tiff")
